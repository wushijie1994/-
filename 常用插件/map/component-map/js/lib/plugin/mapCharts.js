...
MapCharts.prototype = {
    ...
    * */
    ...
    initAreaMap: function () {
        ...
        //集合中添加圆的集合
        //_this.svg.select('#perspective').append(_this.areaMapCircleG);
        //集合中添加文本的集合
        //_this.svg.select('#perspective').append(_this.areaMapTextG);

        //遍历系列数据
        for (var key in series.data) {
            //定义一个属性名的变量，后面会经常用到
            var propName = _this.MAP_PREFIX + key;

            // 区域数据对象
            _this.areaMapData[propName] = series.data[key];
            _this.areaMapData[propName].mapId = key;
            ...
            if ($('#' + propName + '-outline').length > 0) {
                ...
                //往集合中添加对应的地图区域
                _this.areaMapSet[propName].push(_this.areaMap[_this.MAP_PREFIX + key]);

            }
            //判断圆圈是否存在，并且展现
            if ($('#' + propName + '-circle').length > 0) {
                // 区域圆心对象
                _this.areaMapCircle[propName] = _this.svg.select('#' + propName + '-circle');
                //圆心坐标
                var circlePoint = {
                    x: _this.areaMapCircle[propName].attr('cx'),
                    y: _this.areaMapCircle[propName].attr('cy')
                };
                _this.areaMapCircle[propName].attr({
                    cx: _this.matrix.x(circlePoint.x, circlePoint.y),
                    cy: _this.matrix.y(circlePoint.x, circlePoint.y)
                });

                //如果标识展现,则设置对应的样式
                if (series.symbol.itemStyle.normal.show || series.symbol.itemStyle.emphasis.show) {
                    //设置对应的圆的样式
                    _this.setSymbolStyle(_this.areaMapCircle[propName], series.symbol.itemStyle.normal);
     ...
     * */
    setSymbolStyle: function (symbolSvg, style) {
        symbolSvg.attr({
            fill: style.color,
            r: style.size * this.mapRatio,
    ...
    * */
    ...
    createTooltip: function () {
        ...
        //边框
        style += 'border: ' + tooltip.borderWidth + (tooltip.borderWidth > 0 ? 'px' : '') + ' solid ' + tooltip.borderColor + ';';
        //边框圆角
        style += 'border-radius: ' + tooltip.borderRadius + (tooltip.borderRadius > 0 ? 'px' : '') + ';';
        //内边距
        style += 'padding: ' + tooltip.padding + (tooltip.padding > 0 ? 'px' : '') + ';';
        //字体大小
        style += 'font-size:' + tooltip.textStyle.fontSize + 'px' + ';';
        //字体系列
        style += 'font-family:' + tooltip.textStyle.fontFamily + ';';
        //字体粗细
        style += 'font-weight:' + tooltip.textStyle.fontWeight + ';';
        //字体对齐方式
        style += 'text-align:' + tooltip.textStyle.align + ';';
        //字体行高
        style += 'line-height:' + tooltip.textStyle.lineHeight + (tooltip.textStyle.lineHeight > 0 ? 'px' : '') + ';';
        //字体颜色
        style += 'color:' + tooltip.textStyle.color + ';';
        //背景颜色
        style += 'background-color: ' + tooltip.backgroundColor + ';';

        //创建对应的提示框，并添加到body中
        _this.tooltip = $('<div style="' + style + '"></div>').appendTo('body');
    },
    /*
     * 展现提示框
    ...
    showTooltip: function (event, data) {
        ...
        if (_this.tooltip && _this.opts.tooltip.show) {
            //定义提示框文本的值
            var _text = data.name;
            //如果提示框存在内容各时期函数,则格式化对应的文本
            if (_this.opts.tooltip.formatter) {
                _text = _this.opts.tooltip.formatter(data);
            }
            //设置提示框浮层的内容，因为有可能文本是html，所以该处用html
            _this.tooltip.html(_text);
            //更新提示框的位置
    ...
    defaultOption: {
        ...
        tooltip: {
            ...
            textStyle: {
                color: '#fff',  //字体颜色
                align: 'left',    //水平对齐方式,可选为：'left' | 'right' | 'center'
                fontSize: 14,   //字号
        ...
        series: {
            ...
            bgArea: {
                ...
                shadowOffsetX: 2,
                shadowOffsetY: 2,
                opacity: 0.5   //对应的透明度，因为IE不支持阴影颜色为透明度，用opacity去控制对应的透明度
            },
